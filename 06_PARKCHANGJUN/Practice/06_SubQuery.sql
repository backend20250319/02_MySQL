...
SELECT
    DEPT_CODE, SUM(SALARY) AS SAL_SUM -- 부서별 급여 합계
FROM
    EMPLOYEE
WHERE
...
SELECT
    MAX(SAL_SUM) 총합 -- 가장 큰 값
FROM (
         SELECT DEPT_CODE, SUM(SALARY) AS SAL_SUM -- 부서별 급여 합계
         FROM EMPLOYEE
...
FROM
    DEPARTMENT
WHERE
    DEPT_TITLE LIKE '%영업%'; -- D4, D5, D6, D7
...
SELECT
        ...
        E.SALARY          급여,
        NA.DEPT_TITLE     부서명,
        NA.NATIONAL_NAME "(부서의) 국가명"
  FROM
        EMPLOYEE E
            JOIN (...) NA ON E.DEPT_CODE = NA.DEPT_ID
 ORDER BY
        NA.NATIONAL_NAME DESC;
*/
...
FROM
    EMPLOYEE E
        ...
        FROM DEPARTMENT D
                 ...
                 JOIN NATION N ON (L.NATIONAL_CODE = N.NATIONAL_CODE)) AS NA
             ON E.DEPT_CODE = NA.DEPT_ID
ORDER BY
    NA.NATIONAL_NAME DESC;
