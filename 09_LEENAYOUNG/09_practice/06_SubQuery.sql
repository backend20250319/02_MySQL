...
SELECT MAX(SUM_SAL) 총합
    FROM (SELECT SUM(SALARY) SUM_SAL
              FROM employee
              WHERE DEPT_CODE IS NOT NULL
              GROUP BY DEPT_CODE) AS V;
...
SELECT
      ...
      , SALARY 급여
      , SQ.DEPT_TITLE 부서명
      , SQ.NATIONAL_NAME '(부서의) 국가명'
    FROM employee a
    JOIN (SELECT b.DEPT_ID, b.DEPT_TITLE, d.NATIONAL_NAME
          FROM department b
          JOIN location c ON b.LOCATION_ID = c.LOCAL_CODE
          JOIN nation d ON c.NATIONAL_CODE = d.NATIONAL_CODE) AS SQ
    ON a.DEPT_CODE = SQ.DEPT_ID
    ORDER BY 5 DESC;
