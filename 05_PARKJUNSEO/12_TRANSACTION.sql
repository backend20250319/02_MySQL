...
set autocommit = OFF;  # set autocommit = 1  // 자동 커밋 켜두는 코드
-- 커밋 상태값 확인 쿼리
...
START TRANSACTION ;  # 이걸 이용하는 게 좋음
...
START TRANSACTION ;
SELECT menu_price, menu_name, menu_code FROM tbl_menu ;
...
SAVEPOINT strawberry; # 얘가 중요함
UPDATE tbl_menu SET menu_name = '수정된메뉴' WHERE menu_code = 5;
DELETE FROM tbl_menu WHERE menu_code = 7;
--
ROLLBACK TO strawberry; # 이걸 하면 strawberry 밑 update 이전으로 돌아감
--
ROLLBACK ; # 이걸 하면 transaction 이전으로 돌아갈 수 있음
SELECT menu_price, menu_name, menu_code FROM tbl_menu ;