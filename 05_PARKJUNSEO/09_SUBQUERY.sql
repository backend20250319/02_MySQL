...
SELECT  -- main query
 ...
 WHERE
       category_code = (SELECT category_code
                        FROM tbl_menu
                        WHERE menu_name = '열무김치라떼'-- sub query
           );
...
-- 서브쿼리가 다중행을 반환하는 경우, = 연산자를 사용할 수 없다. IN을 사용할 것
SELECT
        *
  FROM tbl_menu
...
-- 스칼라 서브쿼리
-- =========================
-- 결과값이 1개인 상관서브쿼리, 주로 SELECT문에서 사용된다.
-- (단일값을 스칼라값이라고 한다.)
...
SELECT   -- 인라인뷰에 존재하는 컬럼만 참조할 수 있음
        V.menu_name
      , V.category_name
      , V.menu_price
  FROM (SELECT menu_name
             ...
             , menu_price
        FROM tbl_menu a
        ) V;  # 인라인뷰는 보여주고 싶은 것만 담은 테이블을 만들어서 전달하고 싶을 때 자주 사용함
