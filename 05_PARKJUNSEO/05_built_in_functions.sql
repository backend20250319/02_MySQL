...
# 함수의 유형 (단일행처리 함수 vs 그룹함수)
-- 단일행처리 함수 : 각 행마다 반복 호출되어서 호출된 수만큼 결과를 리턴한다.  (모든 행에 대해 처리, 행 수만큼 결과값 반환)
-- 1. 문자처리 함수
-- 2. 숫자처리 함수
-- 3. 날짜시간처리 함수
-- 4. 기타함수
...
# 일반적으로는 단일행처리 함수와 그룹함수를 같이 쓸 수 없음
...
-- FIELD: 찾을 문자열 위치 반환
-- ELT: 해당 위치의 문자열 반환
SELECT ELT(2, '사과', '딸기', '바나나'), FIELD('딸기', '사과', '딸기', '바나나');
...
-- INSTR(기준 문자열, 부분 문자열) // INSTR(string, substring)
-- INSTR: 기준 문자열에서 부분 문자열의 시작 위치 반환

-- LOCATE(부분 문자열, 기준 문자열) //  LOCATE(substring, string [, position])
-- LOCATE: INSTR과 동일하고, position을 지정하면 해당 위치 이후부터 검색

SELECT
       INSTR('사과딸기바나나', '딸기')
     , LOCATE('딸기', '사과딸기바나나'); # INSTR은 앞에서부터 찾고 그 문자의 위치를, LOCATE는 뒤에서부터 찾고 그 문자의 위치를 반환. 결과값은 동일

-- FORMAT(숫자, 소수점 자릿수)
-- FORMAT: 1000단위마다 콤마(,) 표시를 해 주며 소수점 아래 자릿수(반올림)까지 표현한다. 숫자에 ,를 붙이는 건 출력할 때만 하는 게 좋음
-- FORMAT(number, decimal_places)
SELECT FORMAT(123412341234123413.58776, 3);
SELECT menu_code, FORMAT(menu_price, 0) FROM tbl_menu;

-- INSERT(기준 문자열, 위치, 길이, 삽입할 문자열)
-- INSERT: 기준 문자열의 위치부터 길이만큼을 지우고 삽입할 문자열을 끼워 넣는다.
SELECT INSERT('내 이름은 박준서입니다', 7, 3, 'Stephen');
...
-- LPAD: 문자열을 길이만큼 왼쪽으로 늘린 후에 빈 곳을 문자열로 채운다. (문자 왼쪽에 문자열이 길이만큼 되도록 문자열을 채워줌)
-- RPAD: 문자열을 길이만큼 오른쪽으로 늘린 후에 빈 곳을 문자열로 채운다. (문자 오른쪽에 문자열이 길이만큼 되도록 문자열을 채워줌)
...
SELECT RPAD(SUBSTRING('881122-1234567', 1, 8), 14, '*');
SELECT SUBSTRING('안녕하세요 반갑습니다.', 7); # 길이가 없으면 끝까지
...
-- TRUNCATE(숫자, 소수점이하 자리수): 버림값 반환. 소수점이하 자리수는 필수이다. 소수점 이하 자리수까지만 반환 (내림값이 아니라 버림값임)
-- ROUND(숫자, 소수점이하 자릿수): 반올림값 반환
SELECT CEILING(1234.56), FLOOR(1234.56), FLOOR(-0.65);
SELECT ROUND(1234.56), ROUND(12345.567, 2), TRUNCATE(1234.56, 1), TRUNCATE(-0.65, 1);
...
-- RAND()
-- RAND: 0이상 1미만의 실수를 구한다. (소수부)
-- m <= 임의의정수 < n
-- FLOOR((RAND() * (n - m) + m)을 사용  //  FLOOR 덕분에 정수가 됨
...
-- 위 두가지 문법을 지원한다.
-- ADDDATE: 날짜를 기준으로 일수를 더함 (미래)
-- SUBDATE: 날짜를 기준으로 일수를 뺌 (과거)
-- NOW(): 현재 날짜와 시간
-- https://www.w3schools.com/sql/func_mysql_adddate.asp (지원하는 날짜/시간 단위)
...
-- 연월일은 더하거나 뺄 수 없다. 시간을 많이 더하거나 빼면 연월일이 자동으로 바뀜
SELECT NOW(), ADDTIME(NOW(), 1), SUBTIME(NOW(), 1);
SELECT ADDTIME('2025-03-21 15:19:00', '1:0:1');
SELECT SUBTIME('2025-03-21 15:19:00', '4:0:1');
...
SELECT DATEDIFF('2025-03-21', '2022-12-15');  # 827
SELECT DATEDIFF('2022-12-15', '2025-03-21'); # -827
SELECT DATEDIFF('2025-02-21', NOW()), TIMEDIFF('18:00:00', '15:32:12');
SELECT DATEDIFF('2025-09-09', NOW());
...
SELECT EXTRACT(MICROSECOND FROM '2025-03-21 15:39:46.496583');
...
SELECT 3 > 'MAY';  -- 문자는 0으로 변환된다. 따라서 true인 1이 나옴
SELECT 5 > '6MAY', '6MAY' + 1;  # 6까지는 알아서 숫자로 가져오고 나머지 문자열은 그냥 버려버림
SELECT ADDDATE('20250311', 1); -- 날짜형으로 바뀔 수 있는 문자는 DATE형으로 변환된다.
...
SELECT IFNULL(NULL, 'Hello World'), IFNULL('ㅋㅋㅋㅋㅋㅋㅋㅋ', 'Hello World');

-- IF + ISNULL
SELECT IF(ISNULL(NULL), '이름 없음', '이름 있음') AS NAME;

-- IF
-- IF(조건, 참일 때 값, 거짓일 때 값)
SELECT
        menu_name
      , IF(orderable_status = 'Y', '주문 가능', '주문 불가능') '주문가능 여부'
  FROM
        tbl_menu;
...
SELECT
      ...
      , menu_price
      , CASE
            WHEN menu_price < 5000 THEN '저렴한 거'
            WHEN menu_price BETWEEN 5000 AND 10000 THEN '적당한 거'
            WHEN menu_price BETWEEN 10000 AND 20000 THEN '좀 비싼 거'
            ELSE '많이 비싼 거'
        END '가격 수준'
  FROM tbl_menu;
...
-- 내부적으로 합계와 COUNT 정보를 가지고 연산을 한다.
SELECT AVG(menu_price) 가격평균 FROM tbl_menu;

-- 3. COUNT (중요해 중요해 중요해 중요해 중요해 중요해 중요해 중요해 중요해 중요해 중요해 중요해)
-- 테이블에서 조건을 만족하는 행의 개수를 반환하는 함수
-- *은 모든 컬럼 (하나의 행)을 의미한다. 행이 존재하면 1로 카운트
SELECT
        COUNT(*) -- NULL 포함 COUNT (행이 존재하기 때문임)
...
SELECT COUNT(*), category_code FROM tbl_menu; # 이거 오류 (COUNT는 결과가 21로 한 개인데, category_code는 결과가 21개임)
